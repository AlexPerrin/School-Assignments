...
#define DIVS_PER_SEG 20 // REMOVE as soon as possible
...

{
  ...
  vec3* points = new vec3[spline->data.size() * DIVS_PER_SEG + 1];
  ...
  for (float t = 0; t < spline->data.size(); t += 1 / (float)DIVS_PER_SEG) {
    ...
    colours[i] = vec3(1,0,0);
    i++;
  }

  segs->drawSegs(GL_LINE_STRIP, points, colours, i, MV, MVP, lightDir);
