...

{
    ...
    // YOUR CODE HERE
    float HeadX = segHead.x;
    float HeadY = segHead.y;
    float TailX = segTail.x;
    float TailY = segTail.y;
    float landerX = position.x;
    float landerY = position.y;

    vec3 TailToHead = vec3(HeadX - TailX, HeadY - TailY, 0);
    vec3 TailToLander = vec3(landerX - TailX, landerY - TailY, 0);
    vec3 TailToHeadNormalized = TailToHead.normalize();

    float projectTLTH = (TailToHeadNormalized.x * TailToLander.x) + (TailToHeadNormalized.y * TailToLander.y);
    if (projectTLTH < 0)
    {
        return segTail;
    }
    else if (projectTLTH > TailToHead.length())
    {
        return segHead;
    }
    float ratio = projectTLTH / TailToHead.length();

    // Find the point that lies between segTail and segHead
    return vec3(((HeadX - TailX) * ratio) + TailX, ((HeadY - TailY) * ratio) + TailY, 0);
}
...
vec4 Landscape::GroundUnder(vec3 position)
{
    float EndX = 0.0;
    float EndY = 0.0;
    ...
    for (int i = 0; i < numVerts - 1; i++)
    {
        ...
        {
            StartX = landscapeVerts[2 * i];
            StartY = landscapeVerts[2 * i + 1];
            EndX = landscapeVerts[2*(i + 1)];
            EndY = landscapeVerts[2*(i + 1) + 1];
        }
    }
    return vec4(StartX, StartY, EndX, EndY);
}
