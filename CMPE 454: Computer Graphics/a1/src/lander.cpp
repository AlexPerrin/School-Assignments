...

{
  ...
  width = landerDimensions.x;
  height = landerDimensions.y;
...

{
  ...
  mat4 full_transform = worldToViewTransform * translate(position) * rotate(orientation, vec3(0, 0, 1));

  // Push VAO to GPU
  glBindVertexArray(VAO);
  glUniformMatrix4fv(glGetUniformLocation(myGPUProgram->id(), "MVP"), 1, GL_TRUE, &full_transform[0][0]);
  glLineWidth(2.0);
