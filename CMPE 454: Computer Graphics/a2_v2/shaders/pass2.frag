...

{
  ...
  // Texture at the center value
  fragLaplacian = 8.0 * texture(depthSampler, texCoords).xyz;
  
  fragLaplacian -= texture(depthSampler, texCoords + vec2(-texCoordInc.x, texCoordInc.y)).xyz;	// Top-left
  fragLaplacian -= texture(depthSampler, texCoords + vec2(0.0, texCoordInc.y)).xyz;				// Top-center
  fragLaplacian -= texture(depthSampler, texCoords + vec2(texCoordInc.x, texCoordInc.y)).xyz;	// Top-right
  fragLaplacian -= texture(depthSampler, texCoords + vec2(-texCoordInc.x, 0.0)).xyz;			// Center-left
  fragLaplacian -= texture(depthSampler, texCoords + vec2(texCoordInc.x, 0.0)).xyz;				// Center-right
  fragLaplacian -= texture(depthSampler, texCoords + vec2(-texCoordInc.x, -texCoordInc.y)).xyz;	// Bottom-left
  fragLaplacian -= texture(depthSampler, texCoords + vec2(0.0, -texCoordInc.y)).xyz;			// Bottom-center
  fragLaplacian -= texture(depthSampler, texCoords + vec2(texCoordInc.x, -texCoordInc.y)).xyz;	// Bottom-right
}
